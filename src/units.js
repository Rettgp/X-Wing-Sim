...
function Unit(team, pilotid) {
    ...
    for (i = 0; i < u.dial.length; i++) {
        this.dial[i] = {
            move: u.dial[i].move,
            difficulty: u.dial[i].difficulty
        };
    }
...
Unit.prototype = {
    tosquadron: function (s) {
        ...
        var uu = [];
        for (j in upgs)
            if (upgs[j] > -1) uu.push(Upgradefromid(this, upgs[j]));
        ...
        if (!(this.islarge)) {
            ...
            else this.img = s.image("png/" + img, -20 * this.scale, -20 * this.scale, 40 * this.scale, 40 * this.scale).transform('r 90 0 0').attr({
                pointerEvents: "none"
            });


            this.imgsmoke = s.image("png/smoke.gif", -20, -60, 30, 50).transform('r 180 0 0').attr({
                display: "none"
            });
            this.imgflame = s.image("png/out.gif", -15, -40, 20, 40).transform('r 180 0 0').attr({
                display: "none"
            });
        } else {
            if (typeof img == "undefined")
        ...
        this.tohitstats = {};
        this.tohit = s.text(-w, w - 30, "0").attr({
            class: "tohit",
            strokeWidth: 1
        });
        this.meanhit = s.text(-w, w - 15, "0").attr({
            class: "tohit",
            strokeWidth: 1
        });
        this.meanhitsymbol = s.text(-w + 40, w - 15, "0").attr({
            class: "symbols",
            strokeWidth: 0,
            text: "d"
        });
        this.meancrit = s.text(-w, w, "0").attr({
            class: "tohit",
            strokeWidth: 1
        });
        this.meancritsymbol = s.text(-w + 40, w, "0").attr({
            class: "symbols",
            strokeWidth: 0,
            text: "c"
        });
        this.gproba = s.group(this.tohit, this.meanhit, this.meanhitsymbol, this.meancrit, this.meancritsymbol).attr({
            display: "none",
            fontSize: "15",
            stroke: "#fff",
            fill: "#fff",
        ...
        this.gstat = s.group(this.skillbar, this.firebar, this.evadebar, this.shieldbar, this.hullbar, this.gproba).attr({
            pointerEvents: "none"
        });
        this.dialspeed = s.text(2 + w, 3 - w, "").attr({
            class: "dialspeed",
            pointerEvents: "none"
        });
        this.dialdirection = s.text(w + 8, 3 - w, "").attr({
            class: "symbols",
            pointerEvents: "none"
        });
        this.actionicon = s.text(w + 2, -7, "").attr({
            pointerEvents: "none",
            class: "symbols",
            strokeWidth: 0
        });
        ...
        this.geffect = s.group(this.imgflame, this.imgsmoke).attr({
            pointerEvents: "none"
        });
    ...
    setinfo: function (info) {
        ...
        return $(".info").css({
            ...
            pointerEvents: "none"
        }).html(formatstring(info)).appendTo("body").show();
    },
    ...
    rollattackdie: function (n, org, best) {
        var p = [];
        for (var i = 0; i < n; i++) p.push(FACE[ATTACKDICE[this.rand(8)]]);
        return p;
    },
    rolldefensedie: function (n, org, best) {
        var p = [];
        for (var i = 0; i < n; i++) p.push(FACE[DEFENSEDICE[this.rand(8)]]);
        return p;
    },
    rand: function (n) {
    ...
    defenseroll: function (n) {
        ...
        for (f = 0; f <= n; f++) {
            for (e = 0; e <= n - f; e++) {
                ...
                ptot += P[i];
                if (ptot > r) return lock.resolve({
                    dice: n,
                    roll: FE_FOCUS * f + e * FE_EVADE
                }).promise();
            }
        }
        return lock.resolve({
    ...
    getOutlineString: function (m) {
        ...
        return {
            s: "M " + p[0].x + " " + p[0].y + " L " + p[1].x + " " + p[1].y + " " + p[2].x + " " + p[2].y + " " + p[3].x + " " + p[3].y + " Z",
            p: p
        };
    },
    getOutline: function (m) {
        ...
        var t = s.text(w + 8, 3 - w, "8").attr({
            class: "symbols",
            fontSize: "1.3em"
        });
        var g = s.g(t, p).transform(m).attr({
            fill: this.color,
            opacity: 0.3,
            display: "none"
        });
    ...
    getSectorPoints: function (n, m) {
        ...
        return $.map([{
                    x: -(socle + 100 * n) / Math.sqrt(1 + w * w / (w - 3) / (w - 3)),
    ...
    fastgetocollisions: function (mbegin, mend, path, len) {
        ...
        if (typeof path == "undefined") {
            ...
            var x3 = minv.x(x2, y2);
            var y3 = minv.y(x2, y2);
            path = s.path("M " + x1 + " " + y1 + " L " + x3 + " " + y3).appendTo(VIEWPORT).attr({
                display: "none"
            });
    ...
    iscollidingunit: function (m, sh) {
        ...
        if (this.islarge) {
            collision = collision || this.isinoutline(o1, sh, sh.m);
        }
        if (sh.islarge) {
            collision = collision || sh.isinoutline(o2, this, m);
        }
        return collision;
    },
        ...
        for (i = 0; i < this.shipactionList.length; i++) {
            ...
            if (!this.isactiondone(a)) {
                switch (a) {
                    case "CLOAK":
                        if (this.candocloak())
                            al.push(this.newaction(this.addcloak, "CLOAK"));
                        break;
                    case "FOCUS":
                        if (this.candofocus())
                            al.push(this.newaction(this.addfocus, "FOCUS"));
                        break;
                    case "EVADE":
                        if (this.candoevade())
                            al.push(this.newaction(this.addevade, "EVADE"));
                        break;
                    case "TARGET":
                        if (this.candotarget())
                            al.push(this.newaction(this.resolvetarget, "TARGET"));
                        break;
                    case "ARCROTATE":
                        if (this.candoarcrotate())
                            al.push(this.newaction(this.resolvearcrotate, "ARCROTATE"));
                        break;
                    case "BOOST":
                        if (this.candoboost())
                            al.push(this.newaction(this.resolveboost, "BOOST"));
                        break;
                    case "ROLL":
                        if (this.candoroll())
                            al.push(this.newaction(this.resolveroll, "ROLL"));
                        break;
                    case "SLAM":
                        if (isendmaneuver) al.push(this.newaction(this.resolveslam, "SLAM"));
                }
            }
        }
        return al;
    ...
    checkdead: function () {
        if (!this.dead && (this.hull <= 0 || !this.isinzone(this.m))) {
            ...
            if (typeof r[round] == "undefined") r[round] = {
                hits: 0,
                dead: ""
            }
    ...
    modifydefenseroll: function (a, m, n) {
        return m;
    },
    ...
    hashit: function (t) {
        return this.criticalresolved + this.hitresolved > 0;
    },
    ...
    playfiresnd: function () {
        ...
        var start = transformPoint(this.m, {
            x: 0,
            y: -(this.islarge ? 40 : 20)
        });
        var p = s.path("M " + start.x + " " + start.y + " L " + (bb.x + bb.w / 2) + " " + (bb.y + bb.h / 2)).appendTo(VIEWPORT).attr({
            stroke: this.color,
            strokeWidth: 2
        });
    ...
    showpositions: function (gd) {
        ...
        for (i = 0; i < gd.length; i++) {
            var mm = gd[i].m;
            o[i] = this.getOutline(mm).attr({
                title: gd[i].move,
                opacity: 0.4,
                fill: halftone(gd[i].color),
                display: "block",
                class: 'possible'
            }).appendTo(VIEWPORT);
            (function (i) {
    ...
    resolveactionmove: function (moves, cleanup, automove, possible) {
        ...
        var resolve = function (m, k, f) {
            ...
            if (automove) {
                ...
                s.path("M " + tpm.dx + " " + tpm.dy + " L " + gpm.dx + " " + gpm.dy).attr({
                    stroke: this.color,
                    display: TRACE ? "block" : "none",
                    strokeWidth: "20px",
                    strokeLinecap: "round",
                    strokeDasharray: "1, 30",
                    opacity: 0.2,
                    fill: "rgba(0,0,0,0)"
                }).addClass("trace").appendTo(VIEWPORT);
                this.m = m;
            }
        ...
        for (i = moves.length - 1; i >= 0; i--) {
            ...
            if ((possible && (c == YELLOW || c == RED)) || c == GREEN || (possible && !automove)) {
                p = this.getOutline(moves[i]).attr({
                    display: "block"
                }).appendTo(VIEWPORT);
                this.pos.push({
                    ol: p,
    ...
    resolveboost: function (n) {
        this.resolveactionmove(this.getboostmatrix(this.m),
            function (t, k) {
                t.endaction(n, "BOOST");
            }, true, this.canmoveonobstacles("BOOST"));
    },
    candoarcrotate: function () {
        return this.hasmobilearc;
    },
    setarcrotate: function (r) {
    ...
    resolvearcrotate: function (n, noaction) {
        ...
        var self = this;
        for (var i = 0; i < 4; i++) {
            sectors[i] = s.path(aux.call(this, 1, this.m.clone().rotate(i * 90 - this.arcrotation, 0, 0))).attr({
                fill: this.color,
                stroke: this.color,
                opacity: 0.1
            }).appendTo(VIEWPORT);
    ...
    canmoveonobstacles: function (type) {
        return false;
    },
    ...
    resolvetargetnoaction: function (n, noaction) {
        ...
        this.resolveactionselection(p, function (k) {
            ...
            if (noaction == true) this.endnoaction(n, "TARGET");
            else this.endaction(n, "TARGET");
        }.bind(this));
    },
    ...
    completemaneuver: function (dial, difficulty, halfturn, finalm) {
        ...
        if (this.isfireobstructed()) {
            this.log("overlaps obstacle: cannot attack");
        }
        if (this.hascollidedobstacle()) {
            this.log("unit or template overlaps obstacle: no action");
        }
        if (lenC > 0) this.m = m;
        var turn = 0;
        // Animate movement
    ...
    selecttargetforattack: function (wp, enemies) {
        ...
        if (p.length == 0) {
            ...
            this.selectunit(p, function (q, k) {
                if (q[k] != this && this.declareattack(wp, q[k]))
                ...
                case 3:
                    if (this.bombs[2].canbedropped())
                        this.addactivationdial(
                            ...
                            $("<div>").attr({
                                class: "symbols bombs",
                                title: self.bombs[2].name
                            }));
                case 2:
                    if (this.bombs[1].canbedropped())
                        this.addactivationdial(
                            function () {
                            ...
                            $("<div>").attr({
                                class: "symbols bombs",
                                title: self.bombs[1].name
                            }));
                case 1:
                    if (this.bombs[0].canbedropped())
                        this.addactivationdial(
                            function () {
    ...
    showaction: function () {
        if (this.action < this.actionList.length && this.action > -1) {
            ...
            this.actionicon.attr({
                fill: ((this == activeunit) ? c : halftone(c))
            });
        } else this.actionicon.attr({
            text: ""
        });
    },
    showinfo: function () {
        ...
        if (this.istargeted.length > 0) {
            this.infoicon[i++].attr({
                text: A.ISTARGETED.key,
                fill: A.ISTARGETED.color
            });
        }
    ...
    showoutline: function () {
        this.border.attr({
            stroke: ((activeunit == this) ? this.color : halftone(this.color))
        });
    },
    ...
    showstats: function () {
        ...
        if (phase == SELECT_PHASE || phase == CREATION_PHASE) {
            ...
            this.skillbar.attr({
                text: repeat('u', this.getskill())
            });
            this.firebar.attr({
                text: repeat('u', this.weapons[0].getattack())
            });
            this.evadebar.attr({
                text: repeat('u', this.getagility())
            });
            this.hullbar.attr({
                text: repeat('u', this.hull)
            });
            this.shieldbar.attr({
                text: repeat('u', this.shield + this.hull)
            });
    ...
    updatetohit: function (b, wp) {
        ...
        if (!b)
            ...
            for (var i in e) e[i].tohitstats[this.id] = {
                unit: this,
                weapon: wp
            };
    ...
    displaytohit: function (wp) {
        ...
        for (var i in e) {
            ...
            if (hit == 0) u.gproba.attr({
                display: "none"
            });
            else {
                var r = -u.m.split().rotate;
                u.gproba.transform("r " + r + " 0 0").attr({
                    display: "block"
                });
                u.tohit.attr({
                    text: Math.floor(tohit * 100) + "%"
                });
                u.meanhit.attr({
                    text: Math.floor(hit * 100) / 100
                });
                u.meancrit.attr({
                    text: Math.floor(crit * 100) / 100
                });
            }
            u.show();
        }
    ...
    showhitsector: function (b, wp) {
        ...
        if (w.isTurret() || this.isTurret(w)) {
            ...
            if (r0 == 1) {
                for (i = r0; i <= r1; i++) {
                    this.sectors.push(s.path(this.getPrimarySectorString(i, this.m)).attr({
                        fill: this.color,
                        stroke: this.color,
                        opacity: 0.1,
                        pointerEvents: "none"
                ...
                if (typeof w.auxiliary != "undefined") {
                    var aux = w.auxiliary;
                    for (i = r0; i <= r1; i++) {
                        this.sectors.push(s.path(aux.call(this, i, this.m)).attr({
                            fill: this.color,
                            stroke: this.color,
                            opacity: 0.1,
                            pointerEvents: "none"
                ...
                for (i = r0; i <= r1; i++) {
                    this.sectors.push(s.path(this.getPrimarySubSectorString(r0 - 1, i, this.m)).attr({
                        fill: this.color,
                        stroke: this.color,
                        opacity: 0.1,
                        pointerEvents: "none"
                ...
                if (typeof w.subauxiliary != "undefined") {
                    ...
                    for (i = r0; i <= r1; i++) {
                        this.sectors.push(s.path(aux.call(this, r0 - 1, i, this.m.clone())).attr({
                            fill: this.color,
                            stroke: this.color,
                            opacity: 0.1,
                            pointerEvents: "none"
    ...
    showrange: function (b, r0, r1) {
        ...
        if (r0 == 1) {
            for (i = r0; i <= r1; i++)
                this.ranges.push(s.path(this.getRangeString(i, this.m)).attr({
                    fill: this.color,
                    stroke: this.color,
                    opacity: 0.1,
                    pointerEvents: "none"
            ...
            for (i = r0; i <= r1; i++)
                this.ranges.push(s.path(this.getSubRangeString(r0 - 1, i, this.m)).attr({
                    fill: this.color,
                    stroke: this.color,
                    opacity: 0.1,
                    pointerEvents: "none"
    ...
    togglehitsector: function (w) {
        var b;
        if (this.sectors.length + this.ranges.length > 0) b = false;
        else b = true;
        this.showhitsector(b, w);
    },
    togglerange: function () {
        var b;
        if (this.ranges.length > 0) b = false;
        else b = true;
    ...
    isinsector: function (m, n, sh, getSubSectorString, getSectorString, flag) {
        ...
        if (n > 1) o1 = getSubSectorString.call(this, n - 1, n, m);
        else o1 = getSectorString.call(this, n, m);
    ...
    canhavehitocollision: function () {
        return true;
    },
    canhavecriticalocollision: function () {
        return true;
    },
    ...
    animateremovetoken: function (type) {
        ...
        y += p.top + startY;
        $("<div>").addClass("upanim").css({
            left: x,
            top: y
        }).html("<code class='" + type + "'></code>").appendTo("body").show();
    },
    ...
    removehull: function (n) {
        ...
        r[round].hits += n;
        this.log("-%0 %HULL%", n);
        if (this.hull <= this.ship.hull / 2) this.imgsmoke.attr({
            display: "block"
        });
        if (this.hull == 1) {
            this.imgsmoke.attr({
