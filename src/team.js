...
Team.prototype = {
                ...
                if (this.rocks[0] > -1 && this.rocks[1] > -1 && this.rocks[2] > -1) {
                    ...
                    for (var k = 0; k < 3; k++) {
                        ...
                        OBSTACLES[k + (this.team - 1) * 3] =
                            new Rock(this.rocks[k], [o.tx, o.ty, o.alpha],
                                this.team, k);
                    }
                }
    ...
    selectrocks: function () {
        ...
        for (var i = 0; i < ROCKS.length + DEBRISCLOUD.length; i++) {
            if (i < ROCKS.length) {
                g[i] = sa.path(ROCKS[i]).attr({
                    strokeWidth: 3
                });
                if (this.rocks.indexOf(i) > -1)
                    g[i].attr({
                        fill: halftone(this.color),
                        stroke: this.color
                    });
                else g[i].attr({
                    fill: parock,
                ...
                g[i] = sa.path(DEBRISCLOUD[i - ROCKS.length]).attr({
                    strokeWidth: 3
                });
                if (this.rocks.indexOf(i) > -1)
                    g[i].attr({
    ...
    tosquadron: function (s) {
        ...
        */
        ...
        this.history = {
            title: {
                text: UI_translation["Damage taken per turn"]
            },
            axisX: {
                interval: 1,
                title: UI_translation["Turns"]
            },
            axisY: {
                title: UI_translation["Cumulated damage"]
            },
            rawdata: [],
            data: [{
    ...
    toJSON: function () {
        ...
        s.vendor = {
            xwsbenchmark: {
                builder: "Ynot Squadron Benchmark",
                builder_url: "http://ynot6517.github.io/bench/"
            }
        };
        s.version = "0.3.0";
        return s;
    ...
    parseJuggler: function (str, translated) {
        ...
        if ((f & 1) == 1) this.faction = "REBEL";
        else if ((f & 2) == 2) this.faction = "SCUM";
        else this.faction = "EMPIRE";
        this.color = (this.faction == "REBEL") ? RED : (this.faction == "EMPIRE") ? GREEN : YELLOW;
        ...
        for (i = 0; i < pilots.length; i++) {
            ...
            for (j = 0; j < PILOTS.length; j++) {
                ...
                if (PILOTS[j].faction == this.faction) {
                    ...
                    v += pu;
                    vat += pu;
                    if (v.replace(/\'/g, "") == pstr[0]) {
                        pid = j;
                        break;
                    }
                    if (vat.replace(/\'/g, "") == pstr[0]) {
                        pid = j;
                        translated = true;
                        break;
                    }
                    ...
                    if ((translated == true && translate(UPGRADES[k].name).replace(/\'/g, "").replace(/\(Crew\)/g, "") == pstr[j]) ||
                        ...
                        if (authupg.indexOf(UPGRADES[k].type) > -1) {
                            for (f = 0; f < p.upgradetype.length; f++) {
                                ...
                                if (p.upgradetype[f] == UPGRADES[k].type && p.upg[f] == -1) {
                                    p.upg[f] = k;
                                    break;
                                }
                            }
                    ...
                    for (k = 0; k < upg.length; k++) {
                        ...
                        for (var z = 0; z < UPGRADES.length; z++)
                            if (UPGRADES[z].name == UPGRADE_dict[upg[k]]) {
                                for (var f = 0; f < p.upgradetype.length; f++)
                                    if (p.upgradetype[f] == UPGRADES[z].type && p.upg[f] == -1) {
                                        p.upg[f] = z;
                                        break;
                                    }
