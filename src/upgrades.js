...
Bomb.prototype = {
    ...
    getBall: function () {
        ...
        return {
            x: b.x + b.width / 2,
            y: b.y + b.height / 2,
            diam: Math.max(b.width / 2, b.height / 2)
        };
    },
    ...
    resolveactionmove: function (moves, cleanup) {
        ...
        if (moves.length == 1) {
            this.pos[0] = this.getOutline(moves[0]).attr({
            ...
            for (i = 0; i < moves.length; i++) {
                this.pos[i] = this.getOutline(moves[i]).attr({
                    fill: this.unit.color,
                    opacity: 0.7
                });
                (function (k) {
    ...
    display: function (x, y) {
        ...
        this.g.hover(function () {
            ...
            $(".info").css({
                left: x,
                top: y
            }).html(this.name).appendTo("body").show();
        }.bind(this), function () {
            $(".info").hide();
            this.outline.attr({
                stroke: halftone(BLUE)
    ...
    getOutlineString: function (m) {
        ...
        var p1 = transformPoint(m, {
            x: -w - 1,
            y: -w
        });
        var p2 = transformPoint(m, {
            x: w + 1,
            y: -w
        });
        var p3 = transformPoint(m, {
            x: w + 1,
            y: w
        });
        var p4 = transformPoint(m, {
            x: -w - 1,
            y: w
        });
        this.op = [p1, p2, p3, p4];
    ...
    toString: function () {
        ...
        if (this.isactive) {
            ...
            for (i = 0; i < r.length; i++)
                if (r[i].isenemy(this.unit)) break;
            if (i == r.length) this.nofire = true;
            else this.nofire = false;
        }
...
Upgrade.prototype = {
    toString: function () {
        ...
        this.trname = translate(this.name).replace(/\'/g, "&#39;");
        this.left = (this.unit.team == 1);
        if (typeof this.shield != "undefined" && this.shield > 0) this.hasshield = true;
        else this.hasshield = false;
        if (typeof this.focus != "undefined" && this.focus > 0) this.hasfocus = true;
        else this.hasfocus = false;
        if (typeof this.switch != "undefined" && phase == SETUP_PHASE) {
            for (j = 0; j < this.unit.upgrades.length; j++)
                if (this.unit.upgrades[j] == this) break;
            this.hasswitch = {
                uid: this.unit.id,
                uuid: j
            }
