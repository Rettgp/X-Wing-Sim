...
function addredclickchange() {
    var change = function () {
        if ($(this).hasClass("focusreddice")) {
            $(this).removeClass("focusreddice");
            $(this).addClass("hitreddice");
        } else if ($(this).hasClass("blankreddice")) {
            $(this).removeClass("blankreddice");
            $(this).addClass("focusreddice");
        } else if ($(this).hasClass("hitreddice")) {
            $(this).removeClass("hitreddice");
            $(this).addClass("criticalreddice");
        } else if ($(this).hasClass("criticalreddice")) {
            $(this).removeClass("criticalreddice");
            $(this).addClass("blankreddice");
        }
    }
...
function addgreenclickchange() {
    var change = function () {
        if ($(this).hasClass("focusgreendice")) {
            $(this).removeClass("focusgreendice");
            $(this).addClass("evadegreendice");
        } else if ($(this).hasClass("blankgreendice")) {
            $(this).removeClass("blankgreendice");
            $(this).addClass("focusgreendice");
        } else if ($(this).hasClass("evadegreendice")) {
            $(this).removeClass("evadegreendice");
            $(this).addClass("blankgreendice");
        }
    }
...
function enablenextphase() {
    ...
    switch (phase) {
        ...
        case PLANNING_PHASE:
            for (i in squadron)
                if (squadron[i].maneuver < 0 && !squadron[i].isdocked) {
                    ready = false;
                    break;
                }
        ...
        case ACTIVATION_PHASE:
            if (subphase != ACTIVATION_PHASE) {
                ...
                for (i in squadron)
                    if (squadron[i].maneuver > -1 && !squadron[i].isdocked) {
                        ready = false;
                        break;
                    }
...
function displayfactionunits(noreset) {
    ...
    if (faction == "REBEL") $("#dialfilter td[move='SL3']").text(P['TRL3'].key).attr("move", "TRL3");
    else $("#dialfilter td[move='TRL3']").text(P['SL3'].key).attr("move", "SL3");

    for (i in unitlist)
        if (unitlist[i].faction.indexOf(faction) > -1) count++;

    //var tz = Math.round( ( 186 / 2 ) / Math.tan( Math.PI / count ) );
    if (noreset == true) $("#caroussel").html("");
    else $(".caroussel").html("");
    //increment = 360. / count;
    var str;
    ...
    for (u in p) p[u].sort(function (a, b) {
...
function addupgradeaddhandler(u) {
    $("#unit" + u.id + " button.upgrades").click(function (e) {
        ...
        for (var i = 0; i < p.length; i++) {
            ...
            if (typeof upg.attack != "undefined") attacks = [{
                attack: upg.attack,
                lrange: upg.range[0],
                hrange: upg.range[1]
            }];
        ...
        $("#unit" + this.id + " .upglist").html(Mustache.render(TEMPLATES["upglist-creation"], {
            upglist: q
        }));
...
function addrow(team, name, pts, faction, jug, fill, isselection) {
    if (team == 1) {
        $("#squad1").val(jug);
        $("#squad1points").html(pts);
        $("#squad1").attr("data-name", name);
    }
    if (team == 2) {
        $("#squad2").val(jug);
        $("#squad2points").html(pts);
        $("#squad2").attr("data-name", name);
    }
...
function endselection() {
    ...
    if (typeof localStorage[currentteam.name] == "undefined") {
        localStorage[currentteam.name] = JSON.stringify({
            "pts": currentteam.points,
            "faction": currentteam.faction,
            "jug": jug,
            "rocks": currentteam.rocks
        });
    }
...
function importsquad(t) {
    ...
    localStorage[currentteam.name] = JSON.stringify({
        "pts": currentteam.points,
        "faction": currentteam.faction,
        "jug": currentteam.toJuggler(false)
    });
    ...
    switch (phase) {
        ...
        case SETUP_PHASE:
            if ($("#player1 option:checked").val() == "human")
                TEAMS[1].isia = false;
            else TEAMS[1].isia = true;
            TEAMS[2].isia = true
            if ($("#player2 option:checked").val() == "computer")
                TEAMS[2].isia = true;
            else TEAMS[2].isia = false;
...
function setphase(cannotreplay) {
    ...
    switch (phase) {
        ...
        case SETUP_PHASE:
            ...
            if (SETUP.background != "") $(".playmat").css({
                background: "url(" + SETUP.background + ") no-repeat",
            ...
            jwerty.key("alt+3", function () {
                if (!activeunit.iscloaked) {
                    activeunit.addcloaktoken();
                    activeunit.show();
                }
            });
            ...
            jwerty.key("alt+shift+1", function () {
                if (activeunit.focus > 0) activeunit.removefocustoken();
                activeunit.show();
            });
            jwerty.key("alt+shift+2", function () {
                if (activeunit.evade > 0) activeunit.removeevadetoken();
                activeunit.show();
            });
            jwerty.key("alt+shift+3", function () {
                if (activeunit.iscloaked) {
                    activeunit.removecloaktoken();
                    activeunit.show();
                }
            });
            jwerty.key("alt+shift+4", function () {
                if (activeunit.stress > 0) activeunit.removestresstoken();
                activeunit.show();
            });
            jwerty.key("alt+shift+5", function () {
                if (activeunit.ionized > 0) activeunit.removeiontoken();
            });
            jwerty.key("alt+shift+6", function () {
                if (activeunit.tractorbeam > 0) activeunit.removetractorbeamtoken();
            });
...
function permalink(reset) {
    var r = "";
    if (!reset) { /*if (REPLAY!="") r=REPLAY; else*/
        r = ANIM;
    }
    var himg = localStorage["image"];
    var name = localStorage["name"];
...
function fillprobatable() {
    var attacker = {
        focus: $("#focusA").prop("checked") ? 1 : 0,
        reroll: $("#targetA").prop("checked") ? 5 : 0
    };
    var defender = {
        focus: $("#focusD").prop("checked") ? 1 : 0,
...
$(document).ready(function () {
    ...
    P = {
        F0: {
            path: s.path("M 0 0 L 0 0"),
            speed: 0,
            key: "5"
        },
        F1: {
            path: s.path("M 0 0 L 0 -80"),
            speed: 1,
            key: "8"
        },
        F2: {
            path: s.path("M 0 0 L 0 -120"),
            speed: 2,
            key: "8"
        },
        F3: {
            path: s.path("M 0 0 L 0 -160"),
            speed: 3,
            key: "8"
        },
        F4: {
            path: s.path("M 0 0 L 0 -200"),
            speed: 4,
            key: "8"
        },
        F5: {
            path: s.path("M 0 0 L 0 -240"),
            speed: 5,
            key: "8"
        },
        // Turn right
        TR1: {
            path: s.path("M0 0 C 0 -40 15 -55 55 -55"),
            speed: 1,
            key: "6"
        }, // 35 -35
        TR2: {
            path: s.path("M0 0 C 0 -50 33 -83 83 -83"),
            speed: 2,
            key: "6"
        }, // 63 -63
        TRR2: {
        ...
        TR3: {
            path: s.path("M0 0 C 0 -60 45 -105 105 -105"),
            speed: 3,
            key: "6"
        }, // 85 -85
        TRR3: {
        ...
        // Bank right
        BR1: {
            path: s.path("M0 0 C 0 -20 18 -72 38 -92"),
            speed: 1,
            key: "9"
        }, // 24 -58 (+/-14.14)
        BR2: {
            path: s.path("M0 0 C 0 -30 24 -96 54 -126"),
            speed: 2,
            key: "9"
        }, // 40 -92 (+/-14.14)
        SR2: {
        ...
        BR3: {
            path: s.path("M0 0 C 0 -40 29 -120 69 -160"),
            speed: 3,
            key: "9"
        }, // 55 -126 (+/-14.14)
        SR3: {
    ...
    $.when(
        ...
        for (var i = 0; i < UPGRADES.length; i++) {
            ...
            for (var j in UPGRADE_dict)
                if (UPGRADES[i].name == UPGRADE_dict[j]) {
                    found = true;
                    break;
                }
        ...
        var str = "";
        for (i = 0; i < PILOTS.length; i++) {
            if (PILOTS[i].done == true) {
                if (PILOTS[i].unique) u++;
                n++;
            }
            if (!PILOTS[i].done) {
                if (PILOTS[i].unique) str += ", .";
                else str += ", ";
                str += PILOTS[i].name;
            }
        }
        ...
        $(".squadbg > textarea").asuggest(pilots, {
            'delimiters': '^\n',
            'cycleOnTab': true
        });
        $(".squadbg > textarea").asuggest(upgrades, {
            'delimiters': '+',
            'cycleOnTab': true
        });
    });
