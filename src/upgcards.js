...
var UPGRADES = [
        ...
        init: function (sh) {
            ...
            sh.wrap_after("updateactivationdial", this, function () {
                this.addactivationdial(function () {
                    ...
                    this.show();
                }.bind(this), A[ASTROMECH.toUpperCase()].key, $("<div>").attr({
                    class: "symbols"
                }));
        ...
        init: function (sh) {
            ...
            var newdeal = function (c, f, p) {
                ...
                for (var i in this.upgrades) {
                    ...
                    if (u.type == ASTROMECH && u.isactive == true) {
                        k = i;
                        break;
                    }
                }
    ...
    },
    {
        ...
        init: function (sh) {
            ...
            if (typeof Unit.prototype.getswarm == "undefined")
                Unit.prototype.getswarm = function () {
                    return [[], [], []];
                }
    ...
    },
    {
        ...
        action: function (n) {
            var self = this.unit;
            var p = self.selectnearbyally(2, function (t, s) {
                return s.getskill() < t.getskill() && s.candoaction();
            });
            if (p.length > 0) {
    ...
    },
    {
        ...
        candoaction: function () {
            return this.isactive && this.unit.actionsdone.indexOf("ROLL") == -1;
        },
        action: function (n) {
            ...
            sh.adddicemodifier(ATTACK_M, MOD_M, ATTACK_M, this, {
                ...
                f: function (m, n) {
                    ...
                    if (f > 0 && this.mark == round) {
                        if (f > 1) this.unit.log("%0 %FOCUS% -> 1 %CRIT%, %1 %HIT% [%2]", f, f - 1, self.name);
                        else this.unit.log("1 %FOCUS% -> 1 %CRIT% [%1]", f, self.name);
...

    {
        ...
        init: function (sh) {
            ...
            sh.adddicemodifier(ATTACK_M, MOD_M, ATTACK_M, this, {
                req: function (m, n) {
                    return (this.getrange(targetunit) == 3);
                }.bind(sh),
                aiactivate: function (m, n) {
                    return FCH_hit(m) > 0;
                },
        ...
        init: function (sh) {
            ...
            sh.wrap_after("updateactivationdial", this, function () {
                this.addactivationdial(function () {
                    ...
                    this.show();
                }.bind(this), A[ELITE.toUpperCase()].key, $("<div>").attr({
                    class: "symbols"
                }));
                return this.activationdial;
            })
            ...
            sh.wrap_before("beginactivation", this, function () {
                if (this.candoaction() && !this.hasionizationeffect())
                    this.doaction(this.getactionlist(), "").done(function (r) {
                        if (r != null) this.wrap_after("candoendmaneuveraction", self, function () {
                            return false;
                        }).unwrapper("endactivationphase");
                    }.bind(this))
            });
    ...
    },
    {
        ...
        init: function (sh) {
            ...
            sh.adddicemodifier(DEFENSE_M, REROLL_M, DEFENSE_M, this, {
                dice: ["focus"],
                n: function () {
                    if (activeunit.getskill() <= 2) return 2;
                    return 1;
                },
    ...
    },
    {
        ...
        init: function (sh) {
            ...
            sh.adddicemodifier(ATTACK_M, REROLL_M, ATTACK_M, this, {
                dice: ["blank", "focus"],
                n: function () {
                    if (targetunit.getskill() <= 2) return 2;
                    return 1;
                },
    ...
    },
    {
        ...
        init: function (sh) {
            sh.adddicemodifier(DEFENSE_M, REROLL_M, ATTACK_M, this, {
                ...
                req: function () {
                    return this.targeting.indexOf(activeunit) > -1;
                }.bind(sh),
                mustreroll: true,
            });
        }
        ...
        action: function (n) {
            ...
            if (this.isactive)
                this.unit.defenseroll(2).done(function (roll) {
                    ...
                    if (e > 0) str += " +" + e + " %EVADE%";
                    if (str == "") this.log("no effect [%0]", self.name);
                    else this.log(str + " [%0]", self.name);
                    this.endaction(n, "CREW");
                }.bind(this.unit));
            else this.unit.endaction(n, "CREW");
        },
    ...
    },
    {
        ...
        init: function (sh) {
            ...
            sh.addafterattackeffect(this, function (c, h) {
                if (c + h == 0) return;
                var p = targetunit.selectnearbyunits(1, function (t, o) {
                    return o != targetunit;
                });
    ...
    },
    {
        ...
        init: function (sh) {
            var self = this;
            sh.wrap_before("begincombatphase", this, function () {
                var p = this.selectnearbyally(1, function (a, b) {
                    return a.getskill() < b.getskill();
                });
                if (this.canusefocus()) {
                    this.selectunit(p, function (p, k) {
        ...
        init: function (sh) {
            ...
            sh.wrap_after("updateactivationdial", this, function () {
                this.addactivationdial(function () {
                    ...
                    this.show();
                }.bind(this), A[ILLICIT.toUpperCase()].key, $("<div>").attr({
                    class: "symbols"
                }));
                return this.activationdial;
            });
        ...
        init: function (sh) {
            ...
            for (i = 0; i < sh.weapons.length; i++) {
                if (sh.weapons[i].type == CANNON) {
                    ...
                    sh.weapons[i].isTurret = function () {
                        return true;
                    };
        ...
        init: function (sh) {
            ...
            for (var i = 0; i < sh.weapons.length; i++) {
                ...
                if (w.type == TURRET && w.isprimary == false) {
                    turret.push(w);
                    w.wrap_after("isTurret", self, function () {
                        return false;
                    });
                }
            }
    ...
    },
    {
        ...
        init: function (u) {
            var p = s.path("M41.844,-21 C54.632,-21 65,-11.15 65,1 C65,13.15 54.632,23 41.844,23 C33.853,22.912 25.752,18.903 21.904,12.169 C17.975,18.963 10.014,22.806 1.964,23 C-7.439,22.934 -14.635,18.059 -18.94,10.466 C-22.908,18.116 -30.804,22.783 -39.845,23 C-52.633,23 -63,13.15 -63,1 C-63,-11.15 -52.633,-21 -39.845,-21 C-30.441,-20.935 -23.246,-16.06 -18.94,-8.466 C-14.972,-16.116 -7.076,-20.783 1.964,-21 C9.956,-20.913 18.055,-16.902 21.904,-10.17 C25.832,-16.964 33.795,-20.807 41.844,-21 z").attr({
                display: "none"
            });
    ...
    },
    {
        ...
        init: function (sh) {
            ...
            var i, found = -1;
            for (i in squadron)
                if (squadron[i].name == "Nashtah Pup Pilot") {
                    found = i;
                    break;
                }
            if (found > -1) {
        ...
        init: function (sh) {
            ...
            sh.adddicemodifier(ATTACK_M, MOD_M, ATTACK_M, this, {
                ...
                f: function (m, n) {
                    ...
                    if (b > 0) {
                        sh.log("+1 " + s + " [%0]", self.name);
                        m += to;
                    } else if (f > 0) {
                        sh.log("%FOCUS% -> " + s + " [%0]", self.name);
                        m += to - FCH_FOCUS;
                    } else if (h > 0 && to == FCH_CRIT) {
                        sh.log("%HIT% -> %CRIT% [%0]", self.name);
                        m += to - FCH_HIT;
                    }
                    return m;
                }.bind(sh),
    ...
    },
    {
        ...
        init: function (sh) {
            sh.adddicemodifier(ATTACK_M, REROLL_M, ATTACK_M, this, {
                dice: ["blank", "focus"],
                n: function () {
                    return (targetunit.unique == true) ? 2 : 1;
                },
                req: function (attacker, w, defender) {
                    return this.isactive;
                }.bind(this)
    ...
    },
    {
        ...
        init: function (sh) {
            ...
            if (typeof Unit.prototype.getattanni == "undefined")
                Unit.prototype.getattanni = function () {
                    return [];
                }
    ...
    },
    {
        ...
        init: function (sh) {
            ...
            }, this, [sh.weapons[0]], function () {}, function () {
    ...
    },
    {
        ...
        declareattack: function (target) {
            if (!this.isactive) return false;
            var p = this.unit.selectnearbyunits(1, function (a, b) {
                return a != b;
            });
            for (i in p) {
                p[i].addiontoken();
    ...
    },
    {
        ...
        init: function (sh) {
            ...
            sh.wrap_after("preattackroll", this, function (w, t) {
                ...
                if (p.length > 0) {
                    bonus++;
                    p[0].removeevadetoken();
                }
                if (p.length > 1) {
                    bonus++;
                    p[1].removeevadetoken();
                }
