...
f;for(f=0;f<b;f++){e=this.selectdamage();CRITICAL_DECK[e].count--;var g=new Critical(this,e);this.deal(g,FACEUP).then(function(b){switch(b.face){case FACEUP:b.crit.faceup(),this.movelog("c-"+e);case FACEDOWN:this.removehull(1);break;case DISCARD:this.criticals.slice(this.criticals.indexOf(g),1)}this.show()}.bind(this))}},deal:function(b,e){return $.Deferred().resolve({crit:b,face:e}).promise()},gethitrange:function(b,e){return this.isally(e)?0:this.weapons[b].getrange(e)},getenemiesinrange:function(b,
e){var f,g=[];"undefined"==typeof b&&(b=this.weapons);for(f=0;f<b.length;f++)g[f]=b[f].getenemiesinrange(e);return g},getrange:function(b){return this.getoutlinerange(this.m,b).d},getdist:function(b,e){var f=this.getOutlinePoints(b),g=e.getOutlinePoints(e.m),h=-1,l,m;for(l=0;l<f.length;l++)for(m=0;m<g.length;m++){var q=dist(g[m],f[l]);if(q<h||-1==h)h=q}return h},getoutlinerange:function(b,e){var f=util.getOutlineRange(this,e,OBSTACLES);return{d:f.distance,o:f.obstructed}},getrangeallunits:function(){var b=
[[],[],[],[],[]],e;for(e in squadron){var f=squadron[e];f!=this&&(f=this.getrange(f),b[f].push({unit:e}))}return b},updatehtml:function(){$("#"+this.id).html(""+this)}};var IACOMPUTING=0;function IAUnit(){}
...
VIEWPORT.m.clone(),e=$("#svgout").width(),f=$("#svgout").height(),m=0,q=0;f>e?q=(f-e)/2:m=(e-f)/2;var t=Math.max(900/e,900/f),v=this.g.getBBox(),e=$("#svgout").position();$("#playmat").width();$("#playmat").height();f=b.x(v.x,v.y-20)/t;f+=e.left+m;b=b.y(v.x,v.y-20)/t;b+=e.top+q;this.outline.attr({stroke:BLUE});$(".info").css({left:f,top:b}).html(this.name).appendTo("body").show()}.bind(this),function(){$(".info").hide();this.outline.attr({stroke:halftone(BLUE)})}.bind(this));this.g.transform(this.m);
