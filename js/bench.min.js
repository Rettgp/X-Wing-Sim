...
{x:0,y:-(this.islarge?40:20)}),f=s.path("M "+e.x+" "+e.y+" L "+(b.x+b.w/2)+" "+(b.y+b.h/2)).appendTo(VIEWPORT).attr({stroke:this.color,strokeWidth:2}),g=setInterval(function(){f.remove();clearInterval(g)},200);this.movelog("f-"+targetunit.id+"-"+this.activeweapon);"undefined"!=typeof this.weapons[this.activeweapon]&&"undefined"!=typeof this.weapons[this.activeweapon].firesnd?SOUNDS[this.weapons[this.activeweapon].firesnd].play():SOUNDS[this.ship.firesnd].play()},endattack:function(b,e,f){for(i=0;i<
DICES.length;i++)$("."+DICES[i]+"dice").remove();this.show()},endbeingattacked:function(b,e,f){},showpositions:function(b){var e,f=[];for(e=0;e<b.length;e++)f[e]=this.getOutline(b[e].m).attr({title:b[e].move,opacity:.4,fill:halftone(b[e].color),display:"block","class":"possible"}).appendTo(VIEWPORT),function(e){f[e].hover(function(){f[e].attr({stroke:b[e].color,strokeWidth:4})}.bind(this),function(){f[e].attr({strokeWidth:0})}.bind(this))}.call(this,e)},showmeanposition:function(){this.getdial();
...
[[],[],[],[],[]],e;for(e in squadron){var f=squadron[e];f!=this&&(f=this.getrange(f),b[f].push({unit:e}))}return b},updatehtml:function(){$("#"+this.id).html(""+this)}};var Unit=window.Unit||{},Mustache=window.Mustache||{},Critical=window.Critical||{};function Condition(b,e,f){this.name=f;this.org=e;f in TEAMS[b.team].conditions&&TEAMS[b.team].conditions[f].remove();$.extend(this,CONDITIONS[f]);console.log("new condition: "+f+" to "+b.name+" from "+this.org.name);b.conditions[f]=this;TEAMS[b.team].conditions[f]=this;this.unit=b;this.isactive=!0;this.assign(b);activeunit=b;b.show();activeunit=e}
...
type:CREW,points:2}];var allunits=[];function Team(b){this.team=b;this.initiative=this.isia=this.isdead=!1;this.units=[];this.allhits=this.allcrits=this.allevade=this.allred=this.allgreen=0}
...
z[h])<1.2*g&&dist(b[t],z[h])<1.2*g&&(B=z[h].x*x-z[h].y*w+y,0>B*D));h++);0>B*D&&(m=!0,q++)}return 1E4>=g?{distance:1,obstructed:m,obstructionCount:q}:4E4>=g?{distance:2,obstructed:m,obstructionCount:q}:{distance:3,obstructed:m,obstructionCount:q}};//@ sourceMappingURL=bench.js.map
