...
premove:function(){},reavelmaneuver:function(b){},completemaneuver:function(b,e,f,g){var h=P[b].path,l,m;this.maneuverdifficulty=e;if("F0"==b)this.hasmoved=!0,this.handledifficulty(e),this.lastmaneuver=this.maneuver,this.endmaneuver(),this.touching=[];else{var q=h.getTotalLength();this.collision=!1;this.showhitsector(!1);this.islarge&&(q+=40);m=this.m;l=this.getmatrixwithmove(this.m,h,q);var v=this.getcollidingunits(l),w=[];if(0<v.length)for(this.log("collides with %0: no action",v[0].name),this.collision=
!0;0<q&&0<v.length;)w=v,--q,l=this.getmatrixwithmove(this.m,h,q),v=this.getcollidingunits(l);this.drawpathmove(this.m,h,q);for(i=0;i<this.touching.length;i++)v=this.touching[i],v.touching.splice(v.touching.indexOf(this),1);this.touching=w;this.ocollision=this.getocollisions(m,l,h,q);this.isfireobstructed()&&this.log("overlaps obstacle: cannot attack");this.hascollidedobstacle()&&this.log("unit or template overlaps obstacle: no action");0<q&&(this.m=l);var x=0;0<q?(this.hasmoved=!0,$("#activationdial").empty(),
FAST||SOUNDS[this.ship.flysnd].play(),Snap.animate(0,q,function(b){l=this.getmatrixwithmove(m,h,b);this.g.transform(l);this.geffect.transform(l)}.bind(this),TIMEANIM*q/200,mina.linear,function(){this.collision||(b.match(/K\d|SR\d|SL\d/)||1==f?(this.m.rotate(180,0,0),x=180):b.match(/TRL\d/)?(this.m.rotate(-90,0,0),x=-90):b.match(/TRR\d/)&&("undefined"!=typeof g?this.m=g:this.m.rotate(90,0,0),x=90));this.movelog("m-"+b+"-"+(360+x)%360+"-"+Math.floor(q));this.handledifficulty(e);this.lastmaneuver=this.maneuver;
