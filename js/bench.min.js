...
b.length-2))).delay(1500).fadeOut(400);ANIM+="_"+this.id+"-"+b},computepoints:function(){},error:function(b){b=formatstring(b);log("<div><span style='color:red;font-weight:bold;'>**ERROR** ["+this.name+"]</span> "+b+"</div>")},log:function(b,e,f,g){NOLOG||("undefined"!=typeof UI_translation[b]&&(b=UI_translation[b]),"string"==typeof e&&(e=translate(e)),b=b.replace(/%0/g,e),"string"==typeof f&&(f=translate(f)),b=b.replace(/%1/g,f),"string"==typeof g&&(g=translate(g)),b=b.replace(/%2/g,g),b=formatstring(b),
...
if(e!=f)for(var g in squadron)e=squadron[g],this.isally(e)&&this.getskill()>=e.getskill()&&e.showoverflow();0<this.focus&&this.infoicon[b++].attr({text:A.FOCUS.key,fill:A.FOCUS.color});0<this.evade&&this.infoicon[b++].attr({text:A.EVADE.key,fill:A.EVADE.color});1==this.iscloaked&&this.infoicon[b++].attr({text:A.CLOAK.key,fill:A.CLOAK.color});0<this.targeting.length&&6>b&&this.infoicon[b++].attr({text:A.TARGET.key,fill:A.TARGET.color});0<this.istargeted.length&&this.infoicon[b++].attr({text:A.ISTARGETED.key,
...
activeunit!=this&&(e=halftone(e));this.dialspeed.attr({text:P[b.move].speed,fill:e});this.dialdirection.attr({text:P[b.move].key,fill:e})}},clearmaneuver:function(){this.dialspeed.attr({text:""});this.dialdirection.attr({text:""})},beginactivation:function(){this.showmaneuver();this.show()},endactivationphase:function(){},hasionizationeffect:function(){return 0<this.ionized&&!this.islarge||1<this.ionized},begincombatphase:function(){return this.newlock()},beginattack:function(){},toString:function(){if(phase==
SELECT_PHASE||phase==CREATION_PHASE)return this.toString2();var b,e=8;b=squadron.indexOf(this);str=-1==b?"<div class='dead '>":this.isdocked?"<div class='docked'>":"<div>";str+="<div><div class='statskill'>"+this.getskill()+"</div>";t=formatstring(getpilottexttranslation(this,this.faction));str+="<div class='name'>";""!=t&&(str+="<div class='tooltip outoverflow'><span>"+t+"</span></div>");str+="<div>"+translate(this.name)+"</div></div>";text=SHIP_translation[this.ship.name];"undefined"==typeof text&&
(text=this.ship.name);str+="<div><div style='font-size:smaller'><code class='"+this.faction+"'></code>&nbsp;"+text+"</div></div>";e+=2*this.upgrades.length;this.hull+this.shield<=e?(str+="<div class='outoverflow stat'>",str+="<div class='hull'>"+repeat("u ",this.hull)+"</div>",str+="<div class='shield'>"+repeat("u ",this.shield)+"</div></div>"):this.hull>e?(str+="<div class='outoverflow stat'>",str+="<div class='hull'>"+repeat("u ",e)+"</div></div>",this.hull<=2*e?(str+="<div class='outoverflow stat2'>",
str+="<div class='hull'>"+repeat("u ",this.hull-e)+"</div>",this.shield+this.hull<=2*e?str+="<div class='shield'>"+repeat("u ",this.shield)+"</div></div>":(str+="<div class='shield'>"+repeat("u ",2*e-this.hull)+"</div></div>",str+="<div class='outoverflow stat3'>",str+="<div class='shield'>"+repeat("u ",this.shield-2*e+this.hull)+"</div></div>")):(str+="<div class='outoverflow stat2'><div class='hull'>"+repeat("u ",e)+"</div></div>",str+="<div class='outoverflow stat3'><div class='hull'>"+repeat("u ",
this.hull-2*e)+"</div>",str+="<div class='shield'>"+repeat("u ",this.shield)+"</div></div>")):(str+="<div class='outoverflow stat'><div class='hull'>"+repeat("u ",this.hull)+"</div>",str+="<div class='shield'>"+repeat("u ",e-this.hull)+"</div></div>",str+="<div class='outoverflow stat2'><div class='shield'>"+repeat("u ",this.shield-e+this.hull)+"</div></div>");str+="<div><div>";-1<b&&(str+="<div class='usabletokens' style='width:100%'>"+this.getusabletokens()+"</div>");str+="</div></div>";var f=e=
"",g="";b="<td><button class='statevade' onclick='if (!squadron["+b+"].dead&&!squadron["+b+"].isdocked) squadron["+b+"].togglerange();'><span class='val'>"+this.getagility()+"</span><span class='symbols'>^</span></button></td>";e=1==this.team?e+("<tr><td></td>"+b+"</tr>"):e+("<tr>"+b+"<td></td></tr>");for(b=0;b<this.upgrades.length;b++)f+=this.upgrades[b];for(b=0;b<this.criticals.length;b++)g+=this.criticals[b];return str+="<table class='details' style='width:100%'>"+e+f+g+"</table></div>"},canusefocus:function(){return 0<
...
f.subauxiliary,l=g;l<=h;l++)this.sectors.push(s.path(f.call(this,g-1,l,this.m.clone())).attr({fill:this.color,stroke:this.color,opacity:.1,pointerEvents:"none"}).appendTo(VIEWPORT))}}}else{for(l=0;l<this.sectors.length;l++)this.sectors[l].remove();for(l=0;l<this.ranges.length;l++)this.ranges[l].remove();this.ranges=[];this.sectors=[]}this.updatetohit(b,e);this.displaytohit(e)},showrange:function(b,e,f){this.select();if(b)if(1==e)for(b=e;b<=f;b++)this.ranges.push(s.path(this.getRangeString(b,this.m)).attr({fill:this.color,
