...
this.getusabletokens()+"</div>");str+="</div></div>";var f=e="",g="";b="<td><button class='statevade' onclick='if (!squadron["+b+"].dead&&!squadron["+b+"].isdocked) squadron["+b+"].togglerange();'><span class='val'>"+this.getagility()+"</span><span class='symbols'>^</span></button></td>";e=1==this.team?e+("<tr><td></td>"+b+"</tr>"):e+("<tr>"+b+"<td></td></tr>");for(b=0;b<this.upgrades.length;b++)f+=this.upgrades[b];for(b=0;b<this.criticals.length;b++)g+=this.criticals[b];return str+="<table class='details' style='width:100%'>"+
e+f+g+"</table></div>"},canusefocus:function(){return 0<this.focus},canuseevade:function(){return 0<this.evade},canusetarget:function(b){return 0<this.targeting.length&&("undefined"==typeof b||-1<this.targeting.indexOf(b))},getusabletokens:function(){this.focuses=1<this.focus?[this.focus]:[];this.evades=1<this.evade?[this.evade]:[];this.stresses=1<this.stress?[this.stress]:[];this.ionizedes=1<this.ionized?[this.ionized]:[];this.tractorbeames=1<this.tractorbeam?[this.tractorbeam]:[];this.targetedname=
[];this.targetingname=[];for(var b=0;b<this.istargeted.length;b++)this.targetedname[b]=this.istargeted[b].name.replace(/\'/g,"&#39;");for(b=0;b<this.targeting.length;b++)this.targetingname[b]=this.targeting[b].name.replace(/\'/g,"&#39;");return Mustache.render(TEMPLATES.usabletokens,this)},showskill:function(){var b=this.getskill();$("#unit"+this.id+" .statskill").html(b);$("#"+this.id+" .statskill").html(b)},showstats:function(){$("#unit"+this.id+" .statevade .val").html(this.getagility());if(phase==
SELECT_PHASE||phase==CREATION_PHASE)$("#unit"+this.id+" .stathull .val").html(this.ship.hull),$("#unit"+this.id+" .statshield .val").html(this.ship.shield);else{this.skillbar.attr({text:repeat("u",this.getskill())});this.firebar.attr({text:repeat("u",this.weapons[0].getattack())});this.evadebar.attr({text:repeat("u",this.getagility())});this.hullbar.attr({text:repeat("u",this.hull)});this.shieldbar.attr({text:repeat("u",this.shield+this.hull)});var b=8+2*this.upgrades.length;this.hull+this.shield<=
...
0;m<g.length;m++){var q=dist(g[m],f[l]);if(q<h||-1==h)h=q}return h},getoutlinerange:function(b,e){var f=util.getOutlineRange(this,e,OBSTACLES);return{d:f.distance,o:f.obstructed}},getrangeallunits:function(){var b=[[],[],[],[],[]],e;for(e in squadron){var f=squadron[e];f!=this&&(f=this.getrange(f),b[f].push({unit:e}))}return b},updatehtml:function(){$("#"+this.id).html(""+this)}};var IACOMPUTING=0;function IAUnit(){}
...
>>>>>>> b2b3e7eb33e608a363a58d765dae1f4d0e3fb889
