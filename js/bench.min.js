...
h=0;h<=g;h+=5,e++)v=f.getPointAtLength(h),l[e]={x:b.x(v.x,v.y),y:b.y(v.x,v.y)};for(b=0;b<l.length;b++)for(e=0;e<OBSTACLES.length;e++)if(v=OBSTACLES[e],v.type!=NONE&&e!=q.overlap&&e!=this.oldoverlap&&-1==q.template.indexOf(e)&&-1==q.mine.indexOf(v))for(f=v.getOutlineString().p,h=0;h<f.length;h++)if(g=f[h].x-l[b].x,m=f[h].y-l[b].y,100>=g*g+m*m){v.type!=BOMB?q.template.push(e):q.mine.push(OBSTACLES[e]);break}}return q},iscollidingunit:function(b,e){var f=this.getOutlineString(b).s,g=e.getOutlineString(e.m).s,
...
m.weapons[g].getrange(this);0<v&&4>v&&(q=Math.max(q,m.getattackstrength(g,this)))}b+=q;for(g=0;g<this.weapons.length;g++)q=this.weapons[g].getrange(m),0<q&&4>q&&(e=Math.max(e,this.getattackstrength(g,m)));l+=this.getdist(this.m,m)/9E4;f++}}NOLOG=!1;return e-b-l/f},evaluatemoves:function(b,e){this.meanmround=round;var f=this.getdial(),g=0,h=0,l=0,m=0,q,v=RED,w={"#000":0,"#F00":1,"#FF0":2,"#FFF":3,"#0F0":4};NOLOG=!0;var x=(this.m.split().rotate+360+180)%360-180;for(q=0;q<f.length;q++)f[q].path=P[f[q].move].path,
f[q].len=f[q].path.getTotalLength()+(this.islarge?40:0);for(q=0;q<f.length;q++){var y=this.getpathmatrix(this.m,f[q].move);f[q].m=y;if(this.canreveal(f[q])){var z=this.getmovecolor(y,b,e,f[q].path,f[q].len,!0);f[q].color=z;if(z!=RED&&z!=BLACK&&b&&e){var B=RED,E=this.ocollision;for(j=0;j<f.length;j++){var D;if(!("F0"==f[j].move||"RED"==f[j].difficulty&&(this.hasnostresseffect()&&"GREEN"!=f[q].difficulty||"RED"==f[q].difficulty))){var F=this.getmatrixwithmove(y,f[j].path,f[j].len);D=GREEN;this.isinzone(F)?
e&&(this.fastgetocollisions(y,F,f[j].path,f[j].len)?this.ocollision.overlap=1:this.ocollision.overlap=-1,this.hascollidedobstacle()&&(D=YELLOW)):D=RED;w[D]>w[B]&&(B=D)}}this.ocollision=E;w[B]<w[z]&&(f[q].color=B)}}else f[q].color=BLACK;w[v]>w[f[q].color]&&(v=f[q].color);f[q].color!=GREEN&&f[q].color!=WHITE||f[q].move.match(/K\d|SR\d|SL\d|TRL\d|TRR\d/)||(y=y.split(),m++,y.rotate=(y.rotate-x+180)%360-180,g+=y.dx,h+=y.dy,l+=y.rotate)}for(q=0;q<f.length;q++)"F0"==f[q].move&&(f[q].color=v);0==m&&(m=1);
g/=m;h/=m;l/=m;this.meanm=(new Snap.Matrix).translate(g,h).rotate(l+x,0,0);NOLOG=!1},removetarget:function(b){var e;e=b.istargeted.indexOf(this);-1<e&&b.istargeted.splice(e,1);e=this.targeting.indexOf(b);-1<e&&(this.targeting.splice(e,1),this.movelog("t-"+b.id),b.show(),this.show());0==this.targeting.length&&$("#atokens > .xtargettoken").remove()},removeevadetoken:function(){this.animateremovetoken("xevadetoken");this.evade--;this.movelog("e");this.show()},removefocustoken:function(){this.animateremovetoken("xfocustoken");
...
!1;this.showhitsector(!1);this.islarge&&(q+=40);m=this.m;l=this.getmatrixwithmove(this.m,h,q);var v=this.getcollidingunits(l),w=[];if(0<v.length)for(this.log("collides with %0: no action",v[0].name),this.collision=!0;0<q&&0<v.length;)w=v,--q,l=this.getmatrixwithmove(this.m,h,q),v=this.getcollidingunits(l);this.drawpathmove(this.m,h,q);for(i=0;i<this.touching.length;i++)v=this.touching[i],v.touching.splice(v.touching.indexOf(this),1);this.touching=w;this.ocollision=this.getocollisions(m,l,h,q);this.isfireobstructed()&&
this.log("overlaps obstacle: cannot attack");this.hascollidedobstacle()&&this.log("unit or template overlaps obstacle: no action");0<q&&(this.m=l);var x=0;0<q?(this.hasmoved=!0,$("#activationdial").empty(),FAST||SOUNDS[this.ship.flysnd].play(),Snap.animate(0,q,function(b){l=this.getmatrixwithmove(m,h,b);this.g.transform(l);this.geffect.transform(l)}.bind(this),TIMEANIM*q/200,mina.linear,function(){this.collision||(b.match(/K\d|SR\d|SL\d/)||1==f?(this.m.rotate(180,0,0),x=180):b.match(/TRL\d/)?(this.m.rotate(-90,
0,0),x=-90):b.match(/TRR\d/)&&("undefined"!=typeof g?this.m=g:this.m.rotate(90,0,0),x=90));this.movelog("m-"+b+"-"+(360+x)%360+"-"+Math.floor(q));this.handledifficulty(e);this.lastmaneuver=this.maneuver;this.hascollidedobstacle()&&this.resolveocollision(this.ocollision.overlap,this.ocollision.template);if(0<this.ocollision.mine.length)for(i=0;i<this.ocollision.mine.length;i++)this.ocollision.mine[i].detonate(this,!1);this.collision&&this.resolvecollision();this.endmaneuver()}.bind(this))):(this.hasmoved=
...
e){this.wrap_after("afterdefenseeffect",b,e)},isattacktwice:function(){return!1},addattack:function(b,e,f,g,h,l,m){var q=this;"undefined"==typeof l&&(l="endattack");"undefined"==typeof m&&(m=!1);(m?Unit.prototype:this).wrap_after(l,e,function(m,w,x){var v=!1;"removeshield"==l||"warndeath"==l||"endmaneuver"==l?x=activeunit:"undefined"==typeof x&&(x=this);for(i in f)if(f[i].isactive){v=!0;break}if(b.call(q,m,w,x)&&q.noattack<round&&v&&!q.iscloaked&&!q.isfireobstructed()){var z=x.deferred,v=function(){var l,
q=[];this.deferred=z;l="function"==typeof h?h.call(this):this.selectnearbyenemy(3);for(var v in f)f[v].isactive&&0<f[v].getenemiesinrange(l).length&&q.push(f[v]);0==q.length?(this.log("no available target for %0",e.name),this.cleanupattack()):b.call(this,m,w,x)?(this.log("+1 attack [%0]",e.name),"function"==typeof g&&this.wrap_before("resolveattack",this,function(){g.call(this)}.bind(this)).unwrapper("cleanupattack"),this.maxfired++,this.doattack(q,l)):this.cleanupattack()}.bind(q);"endcombatphase"!=
l&&"warndeath"!=l&&phase==COMBAT_PHASE?x.newlock().done(v):v()}})},candoendmaneuveraction:function(){return this.candoaction()&&!this.collision&&!this.hascollidedobstacle()},doendmaneuveraction:function(){return this.doaction(this.getactionlist(!0),"",this.candoendmaneuveraction)},doselection:function(b,e){return this.enqueueaction(b,e)},isactiondone:function(b){return-1!=this.actionsdone.indexOf(b)},doaction:function(b,e,f){"undefined"==typeof f&&(f=this.candoaction);return 0==b.length?(this.log("no action available"),
...
begincombatphase:function(){return this.newlock()},beginattack:function(){},toString:function(){if(phase==SELECT_PHASE||phase==CREATION_PHASE)return this.toString2();var b,e=8;b=squadron.indexOf(this);str=-1==b?"<div class='dead '>":this.isdocked?"<div class='docked'>":"<div>";str+="<div><div class='statskill'>"+this.getskill()+"</div>";t=formatstring(getpilottexttranslation(this,this.faction));str+="<div class='name'>";""!=t&&(str+="<div class='tooltip outoverflow'><span>"+t+"</span></div>");str+=
"<div>"+translate(this.name)+"</div></div>";text=SHIP_translation[this.ship.name];"undefined"==typeof text&&(text=this.ship.name);str+="<div><div style='font-size:smaller'><code class='"+this.faction+"'></code>&nbsp;"+text+"</div></div>";e+=2*this.upgrades.length;this.hull+this.shield<=e?(str+="<div class='outoverflow stat'>",str+="<div class='hull'>"+repeat("u ",this.hull)+"</div>",str+="<div class='shield'>"+repeat("u ",this.shield)+"</div></div>"):this.hull>e?(str+="<div class='outoverflow stat'>",
str+="<div class='hull'>"+repeat("u ",e)+"</div></div>",this.hull<=2*e?(str+="<div class='outoverflow stat2'>",str+="<div class='hull'>"+repeat("u ",this.hull-e)+"</div>",this.shield+this.hull<=2*e?str+="<div class='shield'>"+repeat("u ",this.shield)+"</div></div>":(str+="<div class='shield'>"+repeat("u ",2*e-this.hull)+"</div></div>",str+="<div class='outoverflow stat3'>",str+="<div class='shield'>"+repeat("u ",this.shield-2*e+this.hull)+"</div></div>")):(str+="<div class='outoverflow stat2'><div class='hull'>"+
repeat("u ",e)+"</div></div>",str+="<div class='outoverflow stat3'><div class='hull'>"+repeat("u ",this.hull-2*e)+"</div>",str+="<div class='shield'>"+repeat("u ",this.shield)+"</div></div>")):(str+="<div class='outoverflow stat'><div class='hull'>"+repeat("u ",this.hull)+"</div>",str+="<div class='shield'>"+repeat("u ",e-this.hull)+"</div></div>",str+="<div class='outoverflow stat2'><div class='shield'>"+repeat("u ",this.shield-e+this.hull)+"</div></div>");str+="<div><div>";-1<b&&(str+="<div class='usabletokens' style='width:100%'>"+
this.getusabletokens()+"</div>");str+="</div></div>";var f=e="",g="<td><button class='statevade' onclick='if (!squadron["+b+"].dead&&!squadron["+b+"].isdocked) squadron["+b+"].togglerange();'><span class='symbols'>^</span><span class='val'>"+this.getagility()+"</span></button></td>",h="<td><div class='statshield'><span class='symbols'>*</span><span class='val'>"+this.shield+"</span></div></td>",l="<td><div class='stathull'><span class='symbols'>&</span><span class='val'>"+this.hull+"</span></div></td>";
for(b=0;b<this.upgrades.length;b++)e+=this.upgrades[b];for(b=0;b<this.criticals.length;b++)f+=this.criticals[b];return str+="<table class='details' style='table-layout:fixed; width:100%'>"+("<tr>"+h+l+g)+e+f+"</table></div>"},canusefocus:function(){return 0<this.focus},canuseevade:function(){return 0<this.evade},canusetarget:function(b){return 0<this.targeting.length&&("undefined"==typeof b||-1<this.targeting.indexOf(b))},getusabletokens:function(){this.focuses=1<this.focus?[this.focus]:[];this.evades=
1<this.evade?[this.evade]:[];this.stresses=1<this.stress?[this.stress]:[];this.ionizedes=1<this.ionized?[this.ionized]:[];this.tractorbeames=1<this.tractorbeam?[this.tractorbeam]:[];this.targetedname=[];this.targetingname=[];for(var b=0;b<this.istargeted.length;b++)this.targetedname[b]=this.istargeted[b].name.replace(/\'/g,"&#39;");for(b=0;b<this.targeting.length;b++)this.targetingname[b]=this.targeting[b].name.replace(/\'/g,"&#39;");return Mustache.render(TEMPLATES.usabletokens,this)},showskill:function(){var b=
this.getskill();$("#unit"+this.id+" .statskill").html(b);$("#"+this.id+" .statskill").html(b)},showstats:function(){$("#unit"+this.id+" .statevade .val").html(this.getagility());if(phase==SELECT_PHASE||phase==CREATION_PHASE)$("#unit"+this.id+" .stathull .val").html(this.ship.hull),$("#unit"+this.id+" .statshield .val").html(this.ship.shield);else{this.skillbar.attr({text:repeat("u",this.getskill())});this.firebar.attr({text:repeat("u",this.weapons[0].getattack())});this.evadebar.attr({text:repeat("u",
this.getagility())});this.hullbar.attr({text:repeat("u",this.hull)});this.shieldbar.attr({text:repeat("u",this.shield+this.hull)});var b=8+2*this.upgrades.length;this.hull+this.shield<=b?($("#"+this.id+" .stat .hull").html(repeat("u ",this.hull)),$("#"+this.id+" .stat .shield").html(repeat("u ",this.shield))):this.hull>b?($("#"+this.id+" .stat .hull").html(repeat("u ",b)),this.hull<=2*b?($("#"+this.id+" .stat2 .hull").html(repeat("u ",this.hull-b)),this.shield+this.hull<=2*b?$("#"+this.id+" .stat2 .shield").html(repeat("u ",
this.shield)):($("#"+this.id+" .stat2 .shield").html(repeat("u ",2*b-this.hull)),$("#"+this.id+" .stat3 .shield").html(repeat("u ",this.shield-2*b+this.hull)))):($("#"+this.id+" .stat2 .hull").html(repeat("u ",b)),$("#"+this.id+" .stat3 .hull").html(repeat("u ",this.hull-2*b)),$("#"+this.id+" .stat3 .shield").html(repeat("u ",this.shield)))):($("#"+this.id+" .stat .hull").html(repeat("u ",this.hull)),$("#"+this.id+" .stat .shield").html(repeat("u ",b-this.hull)),$("#"+this.id+" .stat2 .shield").html(repeat("u ",
...
h=0;h<g.length;h++)b+=g[h].replace(/\*/g," + ").replace(/_/g," ")+"<br>",e+=g[h].replace(/\*/g," + ").replace(/_/g," ")+"\n";stype="";TEAMS[1].parseJuggler(e,!1);for(h=1;h<f.rows.length;h++)myTemplate(h,f.rows[h].cellsArray,null,null);SQUADBATTLE.columns.adjust().draw()}},myTemplate=function(b,e,f,g){g=e[0].split(" ");f=g[0].split(":");b=f[0];f=f[1];var h=g[1].split(":");g=h[0];var h=h[1],l=e[1].split("VS"),m=mk2split(l[0]),q=mk2split(l[1]),v="",w="";if(l[0]==SEARCHINGSQUAD){var l=h,x=g,m=q,h=f;g=
b;f=l;b=x}for(q=0;q<m.length-1;q++)w+=m[q].replace(/\*/g," + ").replace(/_/g," ")+"\n",v+=m[q].replace(/\*/g," + ").replace(/_/g," ")+"<br>";TEAMS[2].parseJuggler(w,!1);"en"!=LANG&&(v=TEAMS[2].toJuggler(!0).replace(/\n/g,"<br>"));f=""+f;h=""+h;for(m=0;3>m;m++)3>f.length&&(f="0"+f),3>h.length&&(h="0"+h);"Human"==g&&(h="<b>"+h+"</b>");"Human"==b&&(f="<b>"+f+"</b>");SQUADBATTLE.row.add([h+"-"+f,'<span onclick=\'$(".replay").attr("src","'+e[2]+"\")'>"+v+"</span>"]).draw(!1)},computeurl=function(b,e,f){b=
...
(b=Mustache.render(TEMPLATES.faction,{shipimg:w.img[0],fire:repeat("u",w.fire),evade:repeat("u",w.evade),hull:repeat("u",w.hull),shield:repeat("u",w.shield),diallist:dial2JSON(w.dial),shipname:w.trname,primary:w.weapon_type,actionlist:function(){var b=[];for(h=0;h<w.actionList.length;h++)b[h]=A[w.actionList[h]].key;return b},hastitle:w.hastitle,shipupgrades:q[w.name][0].upgrades,pilots:b}),$("#caroussel").append("<li>"+b+"</li>"))}}}
...
(v=$.parseJSON(localStorage[q]),"undefined"==typeof v.jug||"undefined"==typeof v.pts?delete localStorage[q]:"en"!=LANG?(TEAMS[0].parseJuggler(v.jug,!1),addrow(0,q,v.pts,v.faction,TEAMS[0].toJuggler(!0),!0)):addrow(0,q,v.pts,v.faction,v.jug,!0));$("#squadlist tr:first-child .squadtop").attr("data-intro","Select squad").attr("data-position","bottom");$("#squadlist tr:first-child .logmiddle").attr("data-intro","Battle log").attr("data-position","left");g=[];for(q=0;q<PILOTS.length;q++)v=q,v=translate(PILOTS[q].name),
1==PILOTS[q].ambiguous&&"undefined"!=typeof PILOTS[q].edition&&(v+="("+PILOTS[q].edition+")"),g.push(v.replace(/\'/g,"").replace(/\(Scum\)/g,""));v=[];for(q in UPGRADE_translation)v.push(" "+translate(q).replace(/\'/g,"").replace(/\(Crew\)/g,""));$(".squadbg > textarea").asuggest(g,{delimiters:"^\n",cycleOnTab:!0});$(".squadbg > textarea").asuggest(v,{delimiters:"+",cycleOnTab:!0})})});function util(){}
